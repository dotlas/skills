---
name: closed-plan
description: Produce a closed-loop, fully deterministic implementation checklist from the user's instructions or the conversation — every step executable with zero on-the-fly decisions, no deferred tasks, and no gaps. Design/problem/spec ambiguities are resolved with the user via grill-me; codebase, language, structure, and convention details are decided autonomously. Use when the user says "create a /closed-plan based on our conversation", "create a /closed-plan to implement ...", or wants a plan with no loose ends.
---
# Closed Plan
...
- **When mining the conversation, ignore what was discarded.** If an idea was raised and
  ...
  (resolve it via grill-me, step 3) — never silently plan a discarded idea.

### 2. Triage every open question into exactly one bucket

| Bucket | What it is | How to resolve |
| --- | --- | --- |
| **A — Design / problem** | What the thing should *do* or *be*: requirements, scope, business rules, data semantics, UX behaviour, acceptance criteria, tradeoffs only the user can judge. | **Do NOT guess.** Invoke the `grill-me` skill to interview the user until every Bucket-A question is resolved. |
...
### 3. Resolve Bucket A with grill-me

If any Bucket-A question exists, invoke `grill-me` (one question at a time, each with a
recommended answer) and do not proceed to step 5 until all are resolved.
...
End with a short **Done-when** section: the objective, checkable conditions that mean
the whole task is complete (tests pass, command runs, output matches).
...
- Keep the plan in the response by default.
  Only write it to a file if the user asks.
